...
cfg = {
    ...
    'patience': 300,
    'resume': 'medium_baseline_deeper', # Falas by default, resume training from checkpoint
}
...
if cfg['resume']:
    load_path = os.path.join(cfg['save_dir'], cfg['resume']) # create saving directory
    ckpt_path = f"{load_path}/student_best.ckpt" # the ckpt path of the best student model.
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.8, patience=cfg['patience']//2, verbose=True)
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(valid_loader):
        ...
        scheduler.step(acc) # MEDIUM BASELINE
